...
When `decision_policy.mode` is `typesafe` or `comparative`, Agent-Workflow requires `TYPESAFE_API_KEY` before every Agent Run is prepared and checks again immediately before a prepared headless run is started. Missing credentials are treated as a configuration/readiness error rather than an ordinary semantic fallback. Comparative mode performs the same two run-boundary checks for `agent-workflow-comparative-eval==0.2.0`.
