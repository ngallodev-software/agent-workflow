...
if command -v specgen >/dev/null 2>&1 && specgen version >/dev/null 2>&1; then
  echo "SpecGen available: $(specgen version) ($(command -v specgen))"
  exit 0
fi
...
Install it from GitHub with the maintained installer:
  git clone --depth 1 $REPOSITORY_URL /tmp/specgen-aw
  /tmp/specgen-aw/scripts/install.sh
...
temporary_root="$(mktemp -d "${TMPDIR:-/tmp}/specgen-aw.XXXXXX")"
