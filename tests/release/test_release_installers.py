...
def _write_fake_release(root: Path, *, bad_checksum: bool = False) -> tuple[Path, Path]:
    ...
    release.mkdir()
    bundle_name = "agent-workflow-0.7.7-linux.tar.gz"
    wheel_name = "agent_workflow-0.7.7-py3-none-any.whl"
    staging = root / "agent-workflow-0.7.7-linux"
    staging.mkdir()
...
def test_bootstrap_selects_tagged_bundle_and_verifies_checksum_offline(tmp_path: Path) -> None:
    ...
    result = _run_bootstrap(release, "--version", "v0.7.7", "--python", sys.executable, marker=marker)
    assert result.returncode == 0, result.stdout + result.stderr
    marker_args = marker.read_text(encoding="utf-8").splitlines()
    assert marker_args[-2] == "--wheel"
    assert Path(marker_args[-1]).name == "agent_workflow-0.7.7-py3-none-any.whl"
...
def test_release_workflow_is_tag_only_and_bundle_builder_is_reproducible(tmp_path: Path) -> None:
    ...
    wheel = tmp_path / "agent_workflow-0.7.7-py3-none-any.whl"
    sdist = tmp_path / "agent_workflow-0.7.7.tar.gz"
    ...
    assert first == second
    assert set(first) == {
        "agent-workflow-0.7.7-linux.tar.gz",
        "agent-workflow-0.7.7-wsl2.tar.gz",
        "agent-workflow-0.7.7-macos.tar.gz",
    }
    with tarfile.open(output / "agent-workflow-0.7.7-macos.tar.gz", "r:gz") as archive:
        names = set(archive.getnames())
    assert "agent-workflow-0.7.7-macos/install.sh" in names
    assert "agent-workflow-0.7.7-macos/uninstall.sh" in names
    assert "agent-workflow-0.7.7-macos/agent_workflow-0.7.7-py3-none-any.whl" in names
