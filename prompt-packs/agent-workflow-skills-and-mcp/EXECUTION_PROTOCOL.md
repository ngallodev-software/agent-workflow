...
## 4. Terminal and observability rule

Every qualifying delegation runs through `agent-workflow launch` with a named
session identity that includes project, pack, phase, and ticket. Do not invoke
raw tmux lifecycle commands. If the coordinator has a valid current tmux
context, the CLI creates a visible pane in that window; otherwise it creates a
foregroundable detached named session. Each run must write a live persistent
log.
