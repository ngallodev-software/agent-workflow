...
5. Phase 1 may add strict `tests/future/` installed-product TDD journeys for planned work. Keep them honest expected failures tied to `CHATGPT-TDD-001` and the referenced future backlog items.
6. Preserve unavailable evidence as unavailable. Do not synthesize scores, tokens, costs, provider usage, acceptance, or ledger rows.
7. Run focused tests and release drift checks after each phase. Produce completion evidence with changed paths, commands, exit codes, unresolved issues, and no-drift declaration.
