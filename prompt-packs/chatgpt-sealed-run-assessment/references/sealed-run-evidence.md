...
`agent-workflow ledger deterministic-enforcement-foundations` produced a zero-row ledger because these were implementation/review runs without attached evaluation plans. The exact output is retained in [`deterministic-foundation-ledger.tsv`](deterministic-foundation-ledger.tsv).
