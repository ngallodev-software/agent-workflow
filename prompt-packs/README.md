...
Generated packs must use the current `Workflow -> Task -> Agent Run -> Worker` model and must not introduce a terminal-host dependency into core. New native packs use `agent-workflow/prompt-pack/v1`; shared-contract-generated packs may use `agent-workflow/prompt-pack/v2` with matching bundle provenance. Keep the complete task graph in root `pack.yaml`; do not add per-phase task manifests or workflow metadata in `MANIFEST.json`.
