...
| F13 | Release asset audit and manifest | `scripts/audit-release-assets.py`, `MANIFEST.sha256`, `release-check.sh` | D-CODE / D-ENV | Produce a complete source distribution manifest, SBOM, dependency lock, and provenance statement. Verify wheel and source archive reproducibility independently. | Add signed release attestations, preferably Sigstore or equivalent. Checksums without an authenticated publication channel detect corruption but not malicious replacement. |
...
| F44 | Prompt-pack checksums | `MANIFEST.sha256` | D-CODE | Ban symlinks entirely in prompt packs or include symlink entries and link targets in a canonical manifest format. | **P0 finding:** archive staging preserves symlinks, while `_checksum_files` excludes symlinks. Internal symlinks can therefore affect archive behavior without direct checksum coverage. The current “must not escape root” check is not sufficient for portable integrity. |
