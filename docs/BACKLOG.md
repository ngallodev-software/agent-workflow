...
| MAINT-006 | P0 | Critical | in-progress | Full index verification rejects current MAINT-005 review closure because host historical run directories contain obsolete schemas and incomplete retired artifacts. Host census: 28 legacy metrics and 22 retired-schema records are sealed with final receipts, so an unsealed-only compatibility rule cannot reach them; any follow-up must validate immutable receipt/seal integrity before classifying schema drift. Eight separately invalid sealed records (seven missing required receipt collections, one digest mismatch) remain hard failures and require distinct incident/remediation work. Prove an installed host journey and rerun the blocked reviews. | [SQLite evidence index](SQLITE_EVIDENCE_INDEX_ARCHITECTURE.md#rebuild-and-incremental-synchronization) |
