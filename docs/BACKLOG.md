...
| [`delegation-communication-reliability`](../prompt-packs/delegation-communication-reliability/) | PROC-001 through PROC-005 | PROC-002 is integrated and in review; PROC-001's historical branch is stale and must be reimplemented from current master, while remaining tasks retain their declared readiness and dependencies. |
...
| PROC-001 | P0 | High | ready | The historical `impl/proc-001-r1` branch is not mergeable against current master; reimplement authoritative receipt-derived preflight from current source and prove that failed preflight cannot create a misleading running session. | [`delegation-communication-reliability`](../prompt-packs/delegation-communication-reliability/phase-0/tickets/PROC-001-authoritative-preflight.md) |
...
| Release | Summary |
|---|---|
| 0.3.0 | Sandbox-safe child control bridge, launcher-executable binding, source-snapshot repair, clean wheel builds, and enforceable semantic-version bump checks. PROC-002, PROC-006, and MSG-001 remain separately tracked as in-review until their acceptance evidence is complete; PROC-001 requires a fresh current-master implementation. |
