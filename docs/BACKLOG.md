...
| MAINT-006 | P0 | Critical | ready | Full index verification rejects current MAINT-005 review closure because host historical run directories contain obsolete schemas and incomplete retired artifacts. Classify/quarantine historical evidence without treating it as valid/current or weakening unsafe-path and current-integrity failures; prove an installed host journey and rerun the blocked reviews. | [SQLite evidence index](SQLITE_EVIDENCE_INDEX_ARCHITECTURE.md#rebuild-and-incremental-synchronization) |
