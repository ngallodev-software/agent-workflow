...
| MAINT-003 | P0 | High | in-review | Validated interactive task completion is terminal by default: the host records the durable child-to-parent completion, stops the otherwise-idle executor, seals a final receipt, and closes the tmux pane without manual cleanup. Same-worktree reuse is explicit `--keep-alive`; a disposable live tmux journey proved correlated steering acknowledgement, valid completion, a closed context, a sealed receipt, and a retired pane. | [Execution protocol](references/EXECUTION_PROTOCOL.md#stop-controls) |
