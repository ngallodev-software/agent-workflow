...
| POL-001 | P0 | High | ready | Enforce automatic Codex selection to `gpt-5.6-luna` with only `low`, `medium`, or `high` effort. Larger work must be decomposed rather than promoted to another automatic model; record the effort in launch evidence and reject Codex bypasses. | [`codex-luna-effort-policy`](../prompt-packs/codex-luna-effort-policy/phase-0/tickets/POL-001-luna-effort-policy.md) |
