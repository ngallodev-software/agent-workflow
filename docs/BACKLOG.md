...
| MAINT-002 | P0 | High | in-progress | Make validated interactive task completion terminal by default: the host must record the durable child-to-parent completion, stop the otherwise-idle executor, seal a final receipt, and close the tmux pane without manual cleanup. Keep same-worktree reuse only behind explicit `--keep-alive`; prove completion and correlated steering acknowledgement in a disposable live tmux journey. | [Execution protocol](references/EXECUTION_PROTOCOL.md#stop-controls) |
