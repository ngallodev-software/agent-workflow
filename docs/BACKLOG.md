...
| [`delegation-communication-reliability`](../prompt-packs/delegation-communication-reliability/) | PROC-001 through PROC-005, MAINT-004, MAINT-005 | PROC-001 and PROC-002 remain in review. PROC-003 and PROC-004 are implemented with focused invariant/installed evidence and are in review pending the pack gate and remaining recovery matrix. MAINT-004 repairs review ticket identity; MAINT-005 closes remaining review-launch and completion-template enforcement gaps. |
...
| MAINT-004 | P0 | Critical | ready | Repair review-run completion collection validation: launch contract and child completion must agree on a review ticket identity (or both omit it by contract), while preserving immutable run identity and rejecting mismatched implementation tickets. Add installed review-run coverage proving a read-only review seals a valid completion collection and final receipt. | [Execution protocol](references/EXECUTION_PROTOCOL.md#completion-and-closeout) |
| MAINT-005 | P0 | High | ready | Make review launches intended for acceptance require/derive a recorded tier, and make the generated review completion template/schema contract unambiguous enough to reject string criteria before collection. Prove a tiered read-only review reaches valid collection, sealed receipt, `review`, and `accept`; preserve rejection for missing tier and malformed criteria. | [Execution protocol](references/EXECUTION_PROTOCOL.md#completion-and-closeout) |
| MAINT-003 | P0 | High | in-progress | Validated interactive task completion is terminal by default: the host records the durable child-to-parent completion, stops the otherwise-idle executor, seals a final receipt, and closes the tmux pane without manual cleanup. Same-worktree reuse is explicit `--keep-alive`. Repair the remaining installed orchestrator-inbox journeys, which still expect completed children to expose `idle_reusable` assignment evidence and a first replay advance; retain terminal default semantics and prove the updated registry/inbox behavior. | [Execution protocol](references/EXECUTION_PROTOCOL.md#stop-controls) |
