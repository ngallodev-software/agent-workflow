...
| SUP-002 | P0 | Critical | in-progress | SUP-GATE-0 reproduced two implementation defects: a failed `SAFE-PROBE-STALL-v1` can be retried after the attempt budget, and a successful probe lacks authoritative post-action observation/verification. Repair both, rerun replay/restart/tamper and installed-product evidence, then repeat the gate. | [Supervisor loop](SELF_HEALING_SUPERVISOR_ARCHITECTURE.md#supervisor-lifecycle) |
...
| IDX-002 | P0 | Critical | in-progress | IDX-GATE-0 reproduced a current-revision no-follow failure: a symlinked source was silently omitted without incident evidence and the projection reported current/valid. Repair fail-closed quarantine/incident behavior, then rerun interruption, archive, rebuild-equivalence, and the gate. | [Reconciliation](SQLITE_EVIDENCE_INDEX_ARCHITECTURE.md#rebuild-and-incremental-synchronization) |
...
| ID | Priority | Risk | State | Work and exit evidence | Reference |
|---|---|---:|---|---|---|
| HIER-001 | P0 | Critical | in-progress | HIER-GATE-0 rejected the authority contract: independent duplicate team-lead and root/team identity probes were accepted. Enforce identity uniqueness, rerun adversarial contract checks, then repeat the gate. | [Hierarchical design](HIERARCHICAL_MULTI_TEAM_ORCHESTRATION_DESIGN.md#contracts) |
...
| PROC-004 | P0 | Critical | in-progress | PROC Phase 0 review reproduced `test_pending_reuse_cannot_seal_as_completed`: a pending reuse can still reach completed terminal status. Repair this completion/lifecycle contradiction, then rerun the installed delegation journey and phase gate. | [`delegation-communication-reliability`](../prompt-packs/delegation-communication-reliability/phase-0/tickets/PROC-004-completion-validation.md) |
...
| MAINT-002 | P0 | High | in-review | Validated interactive task completion is terminal by default: the host records the durable child-to-parent completion, stops the otherwise-idle executor, seals a final receipt, and closes the tmux pane without manual cleanup. Same-worktree reuse is explicit `--keep-alive`; a disposable live tmux journey proved correlated steering acknowledgement, valid completion, a closed context, a sealed receipt, and a retired pane. | [Execution protocol](references/EXECUTION_PROTOCOL.md#stop-controls) |
