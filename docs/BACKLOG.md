...
| MAINT-006 | P0 | Critical | in-review | Installed `0.7.9` host rebuild now preserves 205 finite, hash/size-verified historical artifacts outside current evidence (retired schemas/metrics, old envelopes, and pre-collection receipts). Focused invariants and installed journeys pass. Full verification remains correctly false only for the separately owned MAINT-007 integrity cases; rerun the blocked reviews after its disposition boundary is accepted. | [SQLite evidence index](SQLITE_EVIDENCE_INDEX_ARCHITECTURE.md#rebuild-and-incremental-synchronization) |
| MAINT-007 | P0 | Critical | in-progress | Resolve the 17 non-compatible host evidence records without mutating sealed artifacts or treating them as valid/current: one sealed size mismatch, four unsealed legacy contracts, two unsealed stale command catalogs, and ten sealed/dispositioned provenance envelopes. Two rejected implementation attempts proved that (1) sealed MAINT-006-compatible artifacts must retain their existing classification and (2) an incident ledger needs immutable generator/provenance plus a migration/supersession rule: the rejected build wrote 174 stale host incident records. Preserve the installed 17-blocker/207-quarantine baseline; do not trust or delete that stale ledger. | [SQLite evidence index](SQLITE_EVIDENCE_INDEX_ARCHITECTURE.md#rebuild-and-incremental-synchronization) |
