...
| MAINT-006 | P0 | Critical | in-review | Installed `0.7.9` host rebuild preserves the historical MAINT-006 evidence outside current evidence (retired schemas/metrics, old envelopes, and pre-collection receipts). The current rebuild reports 17 integrity errors and 219 quarantined records; full verification remains correctly false globally, while accepted MAINT-007 scoped verification is valid. Focused invariants and installed journeys pass; rerun the remaining blocked reviews. | [SQLite evidence index](SQLITE_EVIDENCE_INDEX_ARCHITECTURE.md#rebuild-and-incremental-synchronization) |
