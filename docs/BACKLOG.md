...
| [`feature-modularization`](../prompt-packs/feature-modularization/) | MAINT-001, PLUG-001, ARC-004 | MAINT-001 is in progress and PLUG-001 is in review; the trusted host now includes explicit enablement, atomic command registration, recovery, catalog provenance, digest-bound installed package resources, and an installed fixture-wheel journey. MOD-GATE-1 remains the only PLUG-001 closure step; ARC-004 remains evidence-gated until the boundary survives real first-party use. |
...
| ID | Priority | Risk | State | Work and exit evidence | Reference |
|---|---|---:|---|---|---|
| HIER-001 | P0 | Critical | in-review | Immutable fixed-depth hierarchy and team-delegation contracts, canonical identity digests, parent capability/budget narrowing, read-only installation, idempotent verification, traversal/symlink/tamper rejection, and an installed-wheel journey are implemented under `agent_workflow.hierarchy`. Complete independent ticket review as part of HIER-GATE-0. | [Hierarchical design](HIERARCHICAL_MULTI_TEAM_ORCHESTRATION_DESIGN.md#contracts) |
| HIER-002 | P0 | Critical | in-review | Append-only fsynced local journals, contiguous per-journal sequences, idempotent source-journal/message imports, strict tamper/truncation/mixed-identity checks, deterministic replay, and digest-sealed read-only team/root receipts are implemented. Receipts bind exact contracts, complete declared journals, contract-required worker outputs, required review/approval evidence, budget use, cross-team bindings, and outcomes; later appends or evidence mutation invalidate verification. Complete HIER-GATE-0 before tmux work. | [Prompt pack phase 0](../prompt-packs/hierarchical-multi-team-orchestration/phase-0/) |
