...
| MAINT-007 | P0 | Critical | ready | Resolve the 17 non-compatible host evidence records without mutating sealed artifacts or treating them as valid/current: one sealed size mismatch, four unsealed legacy contracts, two unsealed stale command catalogs, and ten sealed/dispositioned provenance envelopes. Define a durable incident/disposition authority and scoped review-verification rule; prove genuine corruption remains blocking. | [SQLite evidence index](SQLITE_EVIDENCE_INDEX_ARCHITECTURE.md#rebuild-and-incremental-synchronization) |
