...
For a published release, pin the release explicitly. For version `0.11.10`:

```bash
curl -fsSL https://github.com/ngallodev-software/agent-workflow/releases/download/v0.11.10/install.sh | \
  sh -s -- --version v0.11.10
```
...
runtime, both plugins loaded, SpecGen targeting Agent-Workflow 0.11.10,
