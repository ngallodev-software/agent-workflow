...
| [`comparative-benchmark-scoring-corrections`](../prompt-packs/comparative-benchmark-scoring-corrections/) | BENCH-CORR-001 through BENCH-CORR-010 | Rebased to 0.7.8. Preserves v1 evidence, freezes a corrected major-version scoring contract, expands deterministic evaluation, calibrates every weighted check, prevents source/package/docs drift, and keeps benchmarking inside the built-in feature boundary pending any later ARC-004 extraction. |
