...
def build_parser(plugin_registry: PluginRegistry | None = None) -> argparse.ArgumentParser:
    ...
    index_verify.add_argument(
        "--review-run", "--review", dest="review_session_id",
        help="verify one sealed reviewed run and its direct gate evidence",
    )
    index_disposition = index_commands.add_parser(
        "disposition", help="record a human disposition for an integrity incident"
    )
    index_disposition.add_argument("incident_id")
    index_disposition.add_argument("--actor", required=True)
    index_disposition.add_argument("--decision", choices=("resolved", "reopened"), required=True)
    index_disposition.add_argument("--reason", required=True)
